...
public class SurvivorMenu implements InventoryProvider {
	...
	public void init(Player player, InventoryContents contents) {
		...
		ClickableItem border = ClickableItem.empty(new ItemStackBuilder("inventories.character-selector.survivor-border")
		...
		contents.set(2, 1, border);
		contents.set(0, 0, ClickableItem.empty(new ItemStackBuilder("inventories.character-selector.help")
				.setPlaceholderObject(gamePlayer)
				.build()));
		contents.set(1, 0, ClickableItem.of(new ItemStackBuilder("inventories.character-selector.monsters")
				.setPlaceholderObject(gamePlayer)
				.build(), event -> {
					new MonsterMenu().open(player);
					new SoundPlayer("click").playTo(player);
				}));
		contents.set(2, 0, ClickableItem.empty(new ItemStackBuilder("inventories.character-selector.survivors")
				.setPlaceholderObject(gamePlayer)
				.build()));
		Pagination pagination = contents.pagination();
		ClickableItem[] items = characterManager.getCharactersByRole(Role.SURVIVOR).stream()
				.map(gameCharacter -> {
